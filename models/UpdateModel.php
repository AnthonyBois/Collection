<?php
	...
	class UpdateModel {
		...
		public function updateUser($formData){
			$sql = $this->db->prepare("
				UPDATE user
				SET user_firstname = :firstname, user_name = :name, user_pseudo = :pseudo, user_email = :email, user_password = :password
				WHERE user_id = :id
				");
		...
		public function updateCollection($formData){
			$sql = $this->db->prepare("
				UPDATE collection
				SET collection_title = :title, collection_description = :description, collection_image = :image
